...
# Use install.packages("package name", type = "binary") for now.
library(here)
library(data.table)
library(janitor)
library(dplyr)
...
```
\
Load in shapefiles for England (after downloading manually using the geoportal or similar). In this example we'll use Middle Layer Super Output Areas (MSOA) and Local Authority Districts (LAD).
\
...
```{r}
fill_palette_q <- c(c("#294011", "#4C721D", "#589325", "#88D147", "#D7EFC3"), "grey80") # from DHSCcolours
